...
export default {
    ...
    methods:{
        ...
        //     })
        // },
        asyncPayMent(addressId,orderTotal){
            return  axios.post("/users/payMent",{
                addressId:addressId,
                orderTotal:this.orderTotal
            })
        },
        async payMent(){
            try {
                var addressId =  this.$route.query.addressId;
                let response = await this.asyncPayMent(addressId,this.orderTotal);
                if(!response) {
                ...
                if(response.data.status =="0"){
                    this.$router.push({
                        path:'/orderSuccess?orderId='+response.data.result.orderId
                    })
                    console.log('aysnc ok');
                }
            } catch (error) {
